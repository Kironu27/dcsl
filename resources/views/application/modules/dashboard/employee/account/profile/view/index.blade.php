				          ...
				          <form action="{{ route($hyperlink['page']['update']) }}" method="POST">
									{{ csrf_field() }}
										...
										<div class="columns is-multiline">
											...
											@if(Auth::guard(Session::get('guard'))->user()->role != 'admin')
												...
												<label for="status">Status</label>
													...
													<option value="">--Please Select--</option>
														<option value="active" {{ ((Auth::guard(Session::get('guard'))->user()->status == 'active')?'selected':'') }}>Active</option>
														<option value="inactive" {{ ((Auth::guard(Session::get('guard'))->user()->status == 'inactive')?'selected':'') }}>Inactive</option>
													</select>
												</div>
												<!-- end status -->
